...
{
    SERVER_ILOG(
        "lbm: UME store %d: %s %s success. RegID %d, Flags %x %sOLD[SQN %d] %sNOACKS Source %s",
        ume_registration_ex->store_index,
        std::string(ume_registration_ex->store),
        function_string,
        ume_registration_ex->registration_id,
        ume_registration_ex->flags,
        ume_registration_ex->flags & LBM_SRC_EVENT_UME_REGISTRATION_SUCCESS_EX_FLAG_OLD ? "+" : "-",
        ume_registration_ex->sequence_number,
        ume_registration_ex->flags & LBM_SRC_EVENT_UME_REGISTRATION_SUCCESS_EX_FLAG_NOACKS ? "+" : "-",
...
SenderCallback (int event, void* data, std::string topic_name)
{
    ...
    case LBM_SRC_EVENT_CONNECT:
        SERVER_ILOG("lbm: Source %s connected to %s",
            std::string(topic_name),
            std::string(static_cast<char*>(data)));
        break;

    case LBM_SRC_EVENT_DISCONNECT:
        SERVER_ILOG("lbm: Source %s disconnected from %s",
            std::string(topic_name),
            std::string(static_cast<char*>(data)));
        break;

    case LBM_SRC_EVENT_UME_REGISTRATION_ERROR:
        SERVER_ILOG("lbm: Error registering source with UME store: %s",
            std::string(static_cast<char*>(data)));
        break;
    ...
    case LBM_SRC_EVENT_UME_REGISTRATION_SUCCESS:
        ...
        SERVER_ILOG("lbm: UME store registration success. RegID %u Source %s",
            ume_registration->registration_id,
            std::string(topic_name));
        break;
    ...
    case LBM_SRC_EVENT_UME_DEREGISTRATION_COMPLETE_EX:
        SERVER_ILOG("lbm: UME DEREGISTRATION IS COMPLETE_EX Source %s",
            std::string(topic_name));
        break;
    ...
    case LBM_SRC_EVENT_UME_REGISTRATION_COMPLETE_EX:
        ...
        SERVER_ILOG("lbm: UME registration complete. SQN %d. Flags %x %sQUORUM Source %s",
            ume_registration_complete->sequence_number,
            ume_registration_complete->flags,
            ume_registration_complete->flags & LBM_SRC_EVENT_UME_REGISTRATION_COMPLETE_EX_FLAG_QUORUM ? "+" : "-",
            std::string(topic_name));
        break;
    ...
    case LBM_SRC_EVENT_UME_MESSAGE_NOT_STABLE:
        ...
        {
            SERVER_WLOG("lbm: UME store %d: %s message NOT stable!! SQN %d (cd %s). Flags %x %sLOSS %sTIMEOUT Source %s",
                ume_ack_ex_info->store_index,
                std::string(ume_ack_ex_info->store),
                ume_ack_ex_info->sequence_number,
                msg_clientd,
                ume_ack_ex_info->flags,
                ume_ack_ex_info->flags & LBM_SRC_EVENT_UME_MESSAGE_NOT_STABLE_FLAG_LOSS ? "+" : "-",
                ume_ack_ex_info->flags & LBM_SRC_EVENT_UME_MESSAGE_NOT_STABLE_FLAG_TIMEOUT ? "+" : "-",
                std::string(topic_name));
        }
        else
        {
            SERVER_WLOG("lbm: UME message NOT stable!! SQN %d (cd %s). Flags %x %sLOSS %sTIMEOUT Source %s",
                ume_ack_ex_info->sequence_number,
                msg_clientd,
                ume_ack_ex_info->flags,
                ume_ack_ex_info->flags & LBM_SRC_EVENT_UME_MESSAGE_NOT_STABLE_FLAG_LOSS ? "+" : "-",
    ...
    case LBM_SRC_EVENT_UME_MESSAGE_RECLAIMED_EX:
        ...
        msg_clientd = (ume_ack_ex_info->msg_clientd != nullptr) ? static_cast<char*>(ume_ack_ex_info->msg_clientd) : "nullptr";
        SERVER_ILOG("lbm: UME message reclaimed (ex) - sequence number %d (cd %s). Flags %x Source %s",
            ume_ack_ex_info->sequence_number,
            msg_clientd,
            ume_ack_ex_info->flags,
            std::string(topic_name));
    ...
    case LBM_SRC_EVENT_UME_STORE_UNRESPONSIVE:
        SERVER_ELOG("lbm: UME store: %s Source %s",
            std::string(static_cast<char*>(data)),
            std::string(topic_name));
        break;
    ...
    case LBM_SRC_EVENT_FLIGHT_SIZE_NOTIFICATION:
        ...
        SERVER_ILOG("lbm: Flight Size Notification. Type: %s. Inflight is %s specified flight size Source %s",
            flight_size_notification_type,
            flight_size_notification_state,
            std::string(topic_name));
        break;
...
)
{
    ...
    {
        SERVER_ELOG("lbm: Failed to set ume_session_id, topic: %s error=%s",
            std::string(topic_name),
            std::string(ex.what()));
        return nullptr;
    }
    ...
    {
        SERVER_ELOG("Failed to create Source, topic: %s error=%s",
            std::string(topic_name),
            std::string(ex.what()));
        return nullptr;
    }
    ...
    if (complete_count == 0)
    {
        if (s_use_lbm_transaction_logger)
        {
            SERVER_WLOG("Timed out after waiting %d sec for UME_REGISTRATION_COMPLETE. Will rely on LBM_TRANSACTION_LOGGER to switch to LBM", 
                (int)(UMP_REGISTRATION_TIMEOUT_NS / NANOSECONDS_PER_SECOND));
        }
        else
        {
            SERVER_ELOG("Timed out after waiting %d sec for UME_REGISTRATION_COMPLETE", 
                (int)(UMP_REGISTRATION_TIMEOUT_NS / NANOSECONDS_PER_SECOND));
            delete source;
            return nullptr;
        }
...
static void DataRecvd (const lbm::Message& msg)
{
    ...
    case LBM_MSG_UNRECOVERABLE_LOSS:
        SERVER_ELOG(
            "LOST. topic=%s source=%s seq_num=%x",
            std::string(msg.TopicName()),
    ...
    case LBM_MSG_UNRECOVERABLE_LOSS_BURST:
        SERVER_ELOG(
            "LOST BURST. topic=%s source=%s seq_num=%x",
            std::string(msg.TopicName()),
            std::string(msg.Source()),
            msg.SequenceNumber()
        );
    ...
    case LBM_MSG_BOS:
        SERVER_DLOG("Beginning of transport session. Topic=%s lbm_source=%s", std::string(msg.TopicName()), std::string(msg.Source()));
        break;

    case LBM_MSG_EOS:
        SERVER_DLOG("End of transport session. Topic=%s lbm_source=%s", std::string(msg.TopicName()), std::string(msg.Source()));
        break;

    case LBM_MSG_NO_SOURCE_NOTIFICATION:
        SERVER_ELOG(
            "No sources found for topic=%s.",
            std::string(msg.TopicName())
        );
        break;
    ...
    case LBM_MSG_UME_REGISTRATION_SUCCESS:
    {
        ...
        SERVER_ILOG(
            "UME registration successful. topic=%s source=%s SrcRegID %u RcvRegID %u.",
            std::string(msg.TopicName()),
            std::string(msg.Source()),
            reg->src_registration_id,
    ...
    case LBM_MSG_UME_REGISTRATION_SUCCESS_EX:
    {
        ...
        SERVER_ILOG(
            "UME registration successful. topic=%s source=%s store %u: %s SrcRegID %u RcvRegID %u Flags %x. %s %sNOCACHE %sRPP %s",
            std::string(msg.TopicName()),
            std::string(msg.Source()),
            reg->store_index,
            std::string(reg->store),
            reg->src_registration_id,
            reg->rcv_registration_id,
            reg->flags,
            std::string(sqn),
            reg->flags & LBM_MSG_UME_REGISTRATION_SUCCESS_EX_FLAG_NOCACHE ? "+" : "-",
            reg->flags & LBM_MSG_UME_REGISTRATION_SUCCESS_EX_FLAG_RPP ? "+" : "-",
            std::string(sid)
        );
    }
    break;
    ...
    case LBM_MSG_UME_REGISTRATION_COMPLETE_EX:
    {
        ...
        SERVER_ILOG(
            "UME registration complete. "
            "topic=%s source=%s SQN %x Flags %x. "
            "%sQUORUM "
            "%sRXREQMAX "
            "%s",
            std::string(msg.TopicName()),
            ...
            reg->sequence_number,
            reg->flags,
            reg->flags & LBM_MSG_UME_REGISTRATION_COMPLETE_EX_FLAG_QUORUM ? "+" : "-",
            reg->flags & LBM_MSG_UME_REGISTRATION_COMPLETE_EX_FLAG_RXREQMAX ? "+" : "-",
            std::string(sid)
        );
    }
    break;
    ...
    case LBM_MSG_UME_DEREGISTRATION_SUCCESS_EX:
    {
        ...
        SERVER_ILOG(
            "UME deregistration successful. "
            "topic=%s source=%s store %u: %s "
            ...
            std::string(msg.Source()),
            dereg->store_index,
            std::string(dereg->store),
            dereg->src_registration_id,
            dereg->rcv_registration_id,
            dereg->flags,
            std::string(sqn),
            dereg->flags & LBM_MSG_UME_REGISTRATION_SUCCESS_EX_FLAG_NOCACHE ? "+" : "-",
    ...
    case LBM_MSG_UME_DEREGISTRATION_COMPLETE_EX:
        SERVER_ILOG(
            "UME deregistration complete. topic=%s source=%s",
            std::string(msg.TopicName()),
            std::string(msg.Source())
        );
        break;
    ...
    case LBM_MSG_UME_REGISTRATION_CHANGE:
        SERVER_ILOG(
            "UME registration change. topic=%s source=%s data=%s",
            std::string(msg.TopicName()),
            std::string(msg.Source()),
            std::string(msg.Data())
        );
...
{
    auto v = lbm_version();
    SERVER_ILOG("lbm_version %s", std::string(v));
                ...
                )
{
    ...
    SERVER_ILOG("Creating LBM Context config_file=%s app_name=%s", std::string(config_file), std::string(app_name));
    ...
    SERVER_ILOG("Creating receiver topic=%s", std::string(topic_name));
    ...
    SERVER_ILOG("Creating side channel receiver topic=%s", std::string(topic_name));
    ...
    SERVER_ILOG("Creating side channel sender topic=%s", std::string(topic_name));
    ...
    SERVER_ILOG("Creating receiver EdgeServer topic=%s", std::string(topic_name));
    ...
    SERVER_ILOG("Creating sender Bookie topic=%s", std::string(bookie_topic_name));
...

    {
        ...
        SERVER_ILOG("ConfigXml: use_lbm_transaction_logger=%d", s_use_lbm_transaction_logger);
        if (s_use_lbm_transaction_logger)
        {
            ...
            SERVER_ILOG("Creating LBM Transaction Logger dir_path=%s ump_flight_size=%d", dir_path, ump_flight_size);
...

    {
        ...
        SERVER_ILOG("ConfigXml: ump_ensemble=%s", ump_ensemble);
        lbm_context_t* lbm_context = static_cast<lbm_context_t*>(*s_lbm_context);
        if (ump_ensemble.size() > 0)
        {
            s_use_ump = true;
            SERVER_ILOG("Creating UMP sender topic=%s", std::string(algo_sender_topic_name), ump_ensemble);
            s_algo_sender = AlgoUtil_CreateUMPSender(
                algo_sender_topic_name,
                ump_ensemble.c_str());
            if (ALGOUTIL__HINT_UNLIKELY(s_algo_sender == nullptr))
            {
                SERVER_ELOG("Failed to create sender topic=%s", std::string(algo_sender_topic_name));
                return init_flags;
            }
            else
            {
        ...
        else
        {
            ...
            if (s_use_lbm_transaction_logger)
            {
                SERVER_WLOG("ConfigXml: inconsistent config. s_use_ump=false but use_lbm_transaction_logger=true. Setting use_lbm_transaction_logger=false");
                s_use_lbm_transaction_logger = false;
            }

            SERVER_ILOG("Creating LBM sender topic=%s", std::string(algo_sender_topic_name));
            try
            {
            ...
            {
                SERVER_ELOG("Failed to create sender topic=%s error=%s", std::string(algo_sender_topic_name), std::string(ex.what()));
                return init_flags;
            }
        }
    }
    ...
    SERVER_ILOG("Creating receiver OC topic=%s", std::string(topic_name));
    ...
    SERVER_ILOG("Creating source for User Group topic=%s", USER_GROUP_TOPIC_STRING);
    ...
    try
    {
        ...
        SERVER_ILOG("Creating Triage Action Topic=%s", std::string(triage_action_topic_name));
...
{
    SERVER_ILOG("AlgoServer_ShutdownComm Started");
    ...
    SERVER_ILOG("AlgoServer_ShutdownComm Ended");
    return algoserver__error_none;
}
                                    ...
                                    )
{
    ...
    {
        SERVER_ELOG("Error no algo channel sender for account_id=%llu", connection_id);
    }
    else
    {
        try
        ...
        {
            SERVER_WLOG("AlgoServer_SendAlertExecutionReport: lbm_error=%s use_lbm_transaction_logger=%d", std::string(ex.what()), s_use_lbm_transaction_logger);
        }
    }
}
...
)
{
    ...
    if (channel_sender == nullptr)
    {
        SERVER_ELOG("%s: Cannot create algo sidechannel sender for account_id=%llu", logInfo, account_id);
        try
        {
        ...
        {
            SERVER_WLOG("%s: lbm_error=%s without channel=%llu", logInfo, std::string(ex.what()), account_id);
        }
    }
    else
    {
        ...
        {
            SERVER_WLOG("%s: lbm_error=%s channel=%d", logInfo, std::string(ex.what()), account_id);
        }
    }
}
                     ...
                     )
{
    ...
    {
        SERVER_WLOG(
            "AlgoServer_SendAlert: Encode failed: %s",
            ALGOSERVER_ERROR_STR[algoserver_error]
        );
        return;
    }
...
AlgoServer_SendResponse (algoserver__response* response, const bool isFakeExecutionReport)
{
    ...
    {
        SERVER_ELOG(
            "Encode failed for response=%s. algoserver_error=%s",
            ALGO_RESPONSE_ID_STR[response->id],
            ALGOSERVER_ERROR_STR[algoserver_error]
        );
        free(response);
        return;
    ...
    else
    {
        ...
        if (channel_sender == nullptr)
        {
            SERVER_ELOG("Error no algo channel sender for account_id=%llu", response->connection_id);
        }
        else if (encoded_size == 0)
        {
            SERVER_ELOG("Skip lbm send, encoded_size=0");
        }
        else
        {
            try
            ...
            {
                SERVER_WLOG("Error sending response: : lbm_error=%s use_lbm_transaction_logger", std::string(ex.what()), s_use_lbm_transaction_logger);
            }
        }
    }
    free(response);
...
SendBookieRequest (const char* data, size_t len, lbm::RequestCallback cbk, std::shared_ptr<lbm::Request>& request)
{
    ...
    {
        SERVER_WLOG("Error sending request, lbm_error=%s", std::string(ex.what()));
        return algoserver__error_comm;
    }
...
TransferAlgoFrom (std::string algoserver_id)
{
    ...
    SERVER_ILOG("TransferAlgo: sending book download request for algoserver_id=%s", algoserver_id);
...
TransferAlgoRequestCallback (const lbm::Message& message)
{
    SERVER_ILOG("TransferAlgo response received on LBM thread");
...
DownloadBookRequestCallback (const lbm::Message& message)
{
    ...
    {
        SERVER_ILOG("Duplicate, book download response received, ignoring");
    }
    else
    {
        book_dl_status = algoserver__book_dl_complete;

        SERVER_ILOG("Book download response received");
...
AlgoServer_RequestBookDownload ()
{
    ...
    SERVER_ILOG("Sending book download request");
...
DownloadOrderTagCallback (const lbm::Message& message)
{
    SERVER_ILOG("DownloadOrderTagCallback: OTD ER received on LBM Thread!");
    uuid_t algo_inst_id;
    ...
    if (!uuid_is_null(algo_inst_id))
    {
        ...
        SERVER_ILOG("DownloadOrderTagCallback: Forwarded Order Tags to Server Main Thread: "
                    "algo_inst_id=%s",
                    algo_inst_ttuuid.to_string());
    }
...
AlgoServer_SendOrderTagNewOrderSingle (ttsdk_params_internal& params)
{
    ...
    {
        SERVER_WLOG("AlgoServer_SendOrderTagNewOrderSingle: Error sending OTD NOS: algo_inst_id is missing but OTD error is not fatal: "
                    "algo_inst_id=MISSING");
        return false;
    }
    ...
    {
        SERVER_WLOG("AlgoServer_SendOrderTagNewOrderSingle: Error sending OTD NOS: Unable to find primary_market_id but OTD error is not fatal: "
                    "algo_inst_id=%s",
    ...
    if (sender == nullptr)
    {
        SERVER_WLOG("AlgoServer_SendOrderTagNewOrderSingle: Error sending OTD NOS: Unable to find sender but OTD error is not fatal: "
                    "algo_inst_id=%s",
                    algo_inst_id.to_string());
        return false;
    }
    ...
    SERVER_ILOG("AlgoServer_SendOrderTagNewOrderSingle: Sending OTD NOS: "
                "topic=OC.ORDERTAG.%u "
                "algo_inst_id=%s",
                market_id,
                algo_inst_id.to_string());
    ...
    {
        SERVER_WLOG("AlgoServer_SendOrderTagNewOrderSingle: Error sending OTD NOS: LBM send failed but OTD error is not fatal: "
                    "algo_inst_id=%s "
                    "lbm_exception=%s",
                    algo_inst_id.to_string(),
                    std::string(ex.what()));
...
AlgoServer_EraseOrderTagRequestPtr(const tt::algoutil::ttuuid& algo_inst_id)
{
    ...
    SERVER_ILOG("AlgoServer_EraseOrderTagRequestPtr: Erased ptr: "
                "algo_inst_id=%s "
                "s_ordertag_requests.size()=%d",
                algo_inst_id.to_string(),
                s_ordertag_requests.size());
...
AlgoServer_SendChildCancel (const ttsdk_params_internal& params)
{
    ...
    if (sender == nullptr)
    {
        ...
        SERVER_ELOG("Error sending child cancel: Unable to find sender."
                    " order_id=%s",
                    order_id.to_string());
    ...
    SERVER_ILOG("Sending child cancel on market_id=%u account_id=%llu", market_id, params.account);
    ...
    {
        tt::algoutil::ttuuid order_id(params.ord_id);
        SERVER_ELOG("Error sending child cancel: LBM send failed."
            " order_id=%s error=%s",
            order_id.to_string(),
            std::string(ex.what()));
...
GetOrCreateAlgoChannelSender (unsigned long long account_id)
{
    ...
    if (found == s_algo_senders.end())
    {
        ...
        {
            SERVER_ELOG("Error creating algo sender. account_id=%llu error=%s %s use_lbm_transaction_logger=%d",
                account_id, ALGOUTIL_ERROR_STR[algoutil__error_create_lbm_channel_source], std::string(ex.what()), s_use_lbm_transaction_logger);
            return nullptr;
        }
        SERVER_ILOG("Created algo channel sender. account_id=%llu use_lbm_transaction_logger=%d", account_id, s_use_lbm_transaction_logger);
...
GetOrCreateAlgoSideChannelSender (unsigned long long account_id)
{
    ...
    if (found == s_algo_sidechannel_sprectrum_senders.end())
    {
        ...
        {
            SERVER_ELOG("Error creating algo sender. account_id=%llu error=%s %s useTransactionLogger=%d",
                account_id, ALGOUTIL_ERROR_STR[algoutil__error_create_lbm_channel_source], std::string(ex.what()), s_use_lbm_transaction_logger);
            return nullptr;
        }
        SERVER_ILOG("Created algo channel sender. account_id=%llu useTransactionLogger=%d", account_id, s_use_lbm_transaction_logger);
...
AlgoServer_GetOrCreateChildSender (unsigned int mkt_id, unsigned long long account_id)
{
    ...
    if (found == s_risk_senders.end())
    {
        ...
        {
            SERVER_ELOG("Failed to create Risk topic=%s. error=%s",
                std::string(topic_name),
                std::string(ex.what()));
            return nullptr;
        }
    ...
    SERVER_ILOG("Created child cancel channel sender. account_id=%llu market=%u",
                account_id,
                mkt_id);
...
SimpleSend (char* encoded_response, size_t encoded_size)
{
    ...
    {
        SERVER_WLOG("Error sending response: Lbm send failed. lbm_error=%s", std::string(ex.what()));
    }
}
...
GetOrCreateOrderTagSender (unsigned long long market_id)
{
    ...
    snprintf(oc_ordertag_sender_topic_name,
             ...
             market_id);
    SERVER_ILOG("Creating sender OC Ordertag topic=%s", std::string(oc_ordertag_sender_topic_name));
    try
    {
...
SendToUserGroupChannel (char* encoded_response, size_t encoded_size, unsigned long long user_group_id)
{
    ...
    if (!channel_sender)
    {
        SERVER_ELOG("No Channel sender for user_group_id=%llu", user_group_id);
        return;
    }
    try
    {
        s_user_group_sender->Send(encoded_response, encoded_size, LBM_SRC_BLOCK);
        SERVER_ILOG("Send to %s topic with channel sender for user_group_id=%llu", USER_GROUP_TOPIC_STRING, user_group_id);
    }
    catch (const lbm::LbmError& ex)
    {
        SERVER_ELOG("Failed to send through channel sender for user_group_id=%llu", user_group_id);
    }
}
...
SetLbmSenderMode (const bool useUMP)
{
    ...
    SERVER_ILOG("SetLbmSenderMode to %s %s", useUMP ? "UMP" : "LBM", newState);
    return true;
}
